...
const CONFIG = {
    ...
    MERKLE_RPC: "https://base.merkle.io", // Private Relay
    ...
    // ⚙️ EXECUTION
    LOAN_AMOUNT: ethers.parseEther("600"), // Your 600 ETH target
    GAS_LIMIT: 900000,
    PRIORITY_BRIBE: 15n, // 15% Miner Tip
    WHALE_THRESHOLD: ethers.parseEther("1.0"), // Trigger on 1.0 ETH moves
    MIN_NET_PROFIT: "0.02"
};
...
// Use Public WS for scanning (Save credits)
...
if (cluster.isPrimary) {
    ...
    console.log(`${TXT.bold}${TXT.gold}║    🔱 APEX v38.17.1 | MACH-1 MASTER NODE     ║${TXT.reset}`);
    console.log(`${TXT.bold}${TXT.gold}╚═══════════════════════════════════════════════╝${TXT.reset}\n`);
...
async function runWorker() {
    ...
    process.stdout.write(`${TXT.cyan}[INIT] Nonce: ${nextNonce} | RPCs: ${RPC_POOL.length}${TXT.reset}\n`);
    ...
    function connect() {
        ...
        ws.on('open', () => {
            console.log(`${TXT.green}📡 PUBLIC SCANNER CONNECTED${TXT.reset}`);
    ...
    async function processTransaction(txHash) {
        try {
            ...
            if (netProfit > ethers.parseEther(CONFIG.MIN_NET_PROFIT)) {
                ...
                console.log(`${TXT.gold}💰 PROFIT FOUND: ${ethers.formatEther(netProfit)} ETH (~$${profitUSD.toFixed(2)})${TXT.reset}`);
                ...
                // Send to Private Relay
                const res = await axios.post(CONFIG.MERKLE_RPC, { 
                    jsonrpc: "2.0", id: 1, method: "eth_sendRawTransaction", params: [signedTx] 
                });

                if (res.data.result) {
                    console.log(`${TXT.green}🚀 PRIVATE STRIKE SUCCESS: ${res.data.result}${TXT.reset}`);
                } else {
    ...
    // Start Scanner
    setInterval(() => {
        process.stdout.write(`\r${TXT.dim}[SCANNING]${TXT.reset} ${TXT.cyan}Active${TXT.reset} | ${TXT.silver}Tx: ${scanCount}${TXT.reset} | ${TXT.gold}ETH: $${currentEthPrice.toFixed(2)}${TXT.reset}   `);
        seenHashes.clear();
    }, 5000);
